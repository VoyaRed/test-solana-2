...
def execute_jupiter_transaction(direction, size_sol, price, sl, tp, leverage, margin):
    ...
    if LIVE_WEB3_MODE:
        log_msg = f"🔥 [LIVE WEB3] Broadcasting REAL {action} of {size_sol} SOL position using {margin} SOL margin @ {leverage}x leverage to Jupiter..."
        print(log_msg)
        return True 
    else:
...
def trading_loop():
    global VETO_THRESHOLD, LEVERAGE, MARGIN_SOL
